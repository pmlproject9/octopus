...
type MultiNetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.MultiNetworkPolicy, err error)
...
// List lists all MultiNetworkPolicies in the indexer.
func (s *multiNetworkPolicyLister) List(selector labels.Selector) (ret []*v1alpha1.MultiNetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.MultiNetworkPolicy))
	})
	return ret, err
}
...
type MultiNetworkPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.MultiNetworkPolicy, error)
...
// List lists all MultiNetworkPolicies in the indexer for a given namespace.
func (s multiNetworkPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.MultiNetworkPolicy, err error) {
...
// Get retrieves the MultiNetworkPolicy from the indexer for a given namespace and name.
func (s multiNetworkPolicyNamespaceLister) Get(name string) (*v1alpha1.MultiNetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("multinetworkpolicy"), name)
	}
	return obj.(*v1alpha1.MultiNetworkPolicy), nil
}
